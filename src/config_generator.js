...
function validateTrafficPattern(trafficPattern) {
    if (
        ...
        !TRAFFIC_TYPE_STRINGS.every(key => typeof trafficPattern.distribution[key] === 'number')
    ) {
        return false;
    }
    ...
    if (!isValid) {
        trafficPattern.distribution = correctTrafficPatternDistribution(distribution);
    }
...
async function generateConfigWithRetries(theme, difficulty, numRetries = 3) {
    ...
    while (attempt < numRetries) {
        try {
            ...
            if (!validateConfiguration(config)) throw new Error('Invalid configuration.')
            return config;
        } catch (error) {
            attempt++;
            ...
            if (attempt >= numRetries) {
                throw new Error(`Failed to generate config after ${numRetries} attempts`);
            }
        }
    }
}
