...
function initInputHandlers() {
  if (inputInitialized || !container) return;
    ...
    if (STATE.activeTool === "select") {
      ...
      if (i.type === "service") {
        ...
        if (svc && svc.health < 80 && CONFIG.survival.degradation?.enabled) {
          ...
          if (svc.repair()) {
            addInterventionWarning(
              `Đ«"ő ${svc.type.toUpperCase()} repaired!`,
              "info",
              2000,
            );
            return;
  ...
  container.addEventListener("mousemove", (e) => {
      ...
      if (conn) {
        ...
        showTooltip(
          ...
          `<strong class="text-orange-400">Remove Link</strong><br>
                    <span class="text-gray-300">${fromName}</span> ƒÅ' <span class="text-gray-300">${toName}</span><br>
    ...
    if (i.type === "service") {
      ...
      if (s) {
        ...
        // Highlight hovered service
        if (s.mesh.material.emissive) s.mesh.material.emissive.setHex(0x222222);
        if (s.loadRing) s.loadRing.material.opacity = 0.7;
        cursor = "pointer";
      }
    } else if (i.type === "internet") {
      showTooltip(
        e.clientX + 15,
        e.clientY + 15,
        `<strong class="text-cyan-300">Internet</strong><br>
                <span class="text-gray-400">External traffic source</span>`,
      );
      cursor = STATE.activeTool === "connect" ? "crosshair" : "pointer";
    } else {
      t.style.display = "none";
      STATE.services.forEach((s) => {
        if (s.mesh.material.emissive) s.mesh.material.emissive.setHex(0x000000);
        if (s.loadRing) s.loadRing.material.opacity = 0.3;
      });
    }

    // Change cursor based on hover target and active tool
    if (STATE.activeTool === "connect") {
      ...
      cursor = i.type === "service" ? "crosshair" : "not-allowed";
    } else if (STATE.activeTool === "unlink") {
      const conn = getConnectionAtPoint(e.clientX, e.clientY);
      cursor = conn ? "crosshair" : "not-allowed";
    } else if (
      ["waf", "alb", "lambda", "db", "s3", "sqs", "cache"].includes(
        STATE.activeTool,
      )
    ) {
      cursor = i.type === "ground" ? "copy" : "not-allowed";
    }
...
function createConnection(fromId, toId) {
  ...
  if (t1 === "internet" && (t2 === "waf" || t2 === "alb" || t2 === "edge"))
    ...
    t1 === "alb" &&
    (t2 === "compute" || t2 === "waf" || t2 === "content" || t2 === "intel")
  )
    valid = true;
...
function tick(deltaSeconds = 0) {
  ...
  document.getElementById("rps-display").innerText =
    `${STATE.currentRPS.toFixed(1)} req/s`;
  ...
  if (totalFailures > 0 && failuresPanel) {
    ...
    document.getElementById("failures-total").textContent =
      `${totalFailures} total`;
...
async function registerUser(email, password) {
  try {
    ...
    if (error) {
      document.getElementById("auth-email").classList.add("border-red-400");
      document.getElementById("auth-password").classList.add("border-red-400");
...
async function oauthLogin(provider) {
  try {
    ...
    console.error("Failed to reach OAuth client: ", error);
  }
}
...
function toggleAuthMode() {
  ...
  document.getElementById("auth-title").textContent = window.isSignUpMode
    ? "Create Account"
    : "Sign In";

  document.getElementById("auth-submit-btn").textContent = window.isSignUpMode
    ? "Sign Up"
    : "Sign In";

  document.getElementById("toggle-auth-btn").textContent = window.isSignUpMode
...
async function submitAuth(event) {
  ...
  try {
    ...
    if (!result) throw "Authentication failed!";
...
async function loadTopScores() {
  try {
    const { data, error } = await window.supabase
      .from("user_scores")
      .select("user_id, score, run_time, created_at, email")
...
async function searchLeaderboard(email) {
  if (!email) {
    loadTopScores();
    return;
  }
  ...
  try {
    const { data, error } = await window.supabase
      .from("user_scores")
      .select("user_id, score, run_time, created_at")
      .ilike("user_id", `%${email}%`)
      .order("score", { ascending: false })
      .limit(10);
...
async function saveUserScore() {
  try {
    ...
    if (user) {
      ...
      const runTime = STATE.elapsedGameTime;

      await window.supabase.from("user_scores").insert({
        user_id: user.id,
        run_time: runTime,
        score: score,
        email: user.email,
...
function renderLeaderboard(scores) {
  ...
  if (!scores.length) {
    list.innerHTML = "<div class='text-gray-500 text-center'>No results</div>";
    return;
  }
  ...
  scores.forEach((row, index) => {
    ...
    el.className =
      "flex justify-between items-center bg-gray-800/60 px-4 py-2 rounded-lg border border-gray-700";

    el.innerHTML = `
      <div class="text-cyan-300">
        #${index + 1}
      </div>
      <div class="flex-1 ml-4 text-gray-300 truncate">
        ${row.email.slice(0, 8)}…
      </div>
